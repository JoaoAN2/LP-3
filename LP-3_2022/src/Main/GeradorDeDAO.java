...
class GeradorDeDAO {
    ...
    GeradorDeDAO(String className, List<String> atributos) {
        ...
        if (pk[0].equals("int")) {
            cg.add("    public int autoId" + className + "() {\n"
                    + "        Integer a = (Integer) em.createQuery(\"SELECT MAX(e." + pk[1] + ") FROM " + className + "e \").getSingleResult();\n"
                    + "        if(a != null) {\n"
                    + "            return a + 1;\n"
                    + "        } else {\n"
                    + "            return 1;\n"
        ...
        for (int i = 0; i < atributos.size(); i++) {
            ...
            if (aux[0].equals("String")) {
                cg.add("    public List<" + className + "> listBy" + stringTools.firstLetterToUpperCase(aux[1]) + "(" + aux[0] + " " + stringTools.firstLetterToLowerCase(aux[1]) + ") {\n"
                        + "        return em.createQuery(\"SELECT e FROM " + className + " e WHERE e." + aux[1] + " LIKE :" + aux[1] + "\").setParameter(\"" + aux[1] + "\", \"%\" + " + aux[1] + " + \"%\").getResultList();\n"
                        + "    }\n");
            }

            cg.add("    public List<" + className + "> listInOrder" + stringTools.firstLetterToUpperCase(aux[1]) + "() {\n"
                    + "        return em.createQuery(\"SELECT e FROM " + className + " e ORDER BY e." + aux[1] + "\").getResultList();\n"
                    + "    }\n");
        }
